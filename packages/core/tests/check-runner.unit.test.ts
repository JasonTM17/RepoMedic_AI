...
describe('Check Runner', () => {
  ...
  it('runChecks with a fast passing command (node --version) returns status "passed"', async () => {
    const results = await runChecks(tmp, [{ name: 'test-pass', command: 'node', args: ['--version'] }]);
  ...
  it('runChecks with a failing command returns status "failed"', async () => {
    const results = await runChecks(tmp, [{ name: 'test-fail', command: 'node', args: ['--eval', 'process.exit(1)'] }]);
    expect(results).toHaveLength(1);
    expect(results[0]!.status).toBe('failed');
  });

  it('runChecks returns array of CheckResult with correct shape', async () => {
    const results = await runChecks(tmp, [{ name: 'test-shape', command: 'node', args: ['--version'] }]);
    expect(results).toHaveLength(1);
  ...
  it('output is truncated at 8192 chars if over limit', async () => {
    ...
    const results = await runChecks(tmp, [{ name: 'test-trunc', command: 'node', args: ['--eval', script] }]);
