...
export async function gitLogTool(input: GitLogInput): Promise<ToolResult<GitLogResult>> {
  const n = input.maxCommits ?? 50;
  try {
    const result = await boundedExec('git', ['log', '--oneline', `-n`, String(n)], { cwd: input.root, timeoutMs: 10_000 });
