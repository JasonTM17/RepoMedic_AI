...
export function createProgram(): Command {
  ...
  program
    ...
    .description('Triage a local git repository for bugs and optionally apply patches')
    .option('--dry-run', 'Plan only — do not apply any patches', false)
    .option('--model <backend>', 'Model backend: fake or openai', 'fake')
    .option('--allowlist <paths>', 'Comma-separated path allowlist (relative to repo root)', '.')
    .option('--max-retries <n>', 'Maximum patch retry attempts', '3')
    .option('--issue <description>', 'Issue description to investigate', 'Investigate and fix all bugs')
